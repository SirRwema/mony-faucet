...
# substrate RPC node
node_rpc = "wss://central.themony.network:443"
...
async def nine_nine(ctx, arg):
    if (ctx.channel.type == "private"):
        ...
        substrate = SubstrateInterface(
            url=node_rpc,
            address_type=42,
            type_registry_preset='mony-testnet-node'
        )
        ...
        await ctx.send(ctx.author.mention + " Awesome, you just received 100 dMNY, it has no real value it's only the testnet token :) " +  reply['extrinsic_hash'] + str(ctx.channel.type))
